# Website content analysis

Comparison of 20 wikipedia articles.<br>
Articles are in polish.

## Goals

The goal is to calculate cosine similarities and nouns frequencies for preselected wikipedia sites.<br>
Sites are preselected in such a way to have a few sites from a few topics not related to each other
and see how the final similarity will look like.<br>

Topics:
* LLM models
* ML libraries
* books written by Pratchett
* astronomical objects
* astrophysics topics
* my favourite photos :) -> Pale Blue Dot and The Blue Marble
* Greek mythology / books written by Homer

### Fetching and preprocessing content

Articles are fetched from wikipedia using `aiohttp` and `beautifulsoup` lib is used to extract text from content.<br>
Embeddings are created in chromaDB using OpenAI.<br>

### Finding similarities between contents

#### `distance.cosine` from `scipy.spatial`

`distance.cosine` is used to calculate similarities for all urls combinations.

#### top 5 results from chromaDB search

Last part of url is used as query search and top 5 results are presented.

It is a good opportunity to see how search based on similarities can be sometimes misleading.<br>

For example I chose the following articles in my test set:
* https://pl.wikipedia.org/wiki/Dysk_akrecyjny (astrophysics)
* https://pl.wikipedia.org/wiki/Zbrojni (Discworld by Pratchett)
* https://pl.wikipedia.org/wiki/Muzyka_duszy (Discworld by Pratchett)

And top5 for the 1st position:
```json
"chromadb_search": {
        "query_text": "Dysk akrecyjny",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "https://pl.wikipedia.org/wiki/Zbrojni",
          "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "https://pl.wikipedia.org/wiki/Agamemnon"
        ]
      }
```

### List of nouns from content with their frequencies

`spacy` library is used and `pl_core_news_sm` language package is downloaded.<br>
`stanza` library seems to be better in correct polish nouns recognition but calculations take a lot of time.<br>
This is the reason why `spacy` is chosen for those calculations.<br>
Nouns having at least 2 occurrences are displayed.

## Swagger docs view for the stats endpoint

![Swagger docs view for the stats endpoint](misc/urls-stats-swagger.jpg)

## Response generated by stats endpoint

```json
{
  "stats": [
    {
      "url1": "https://pl.wikipedia.org/wiki/ChatGPT",
      "chromadb_search": {
        "query_text": "ChatGPT",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/ChatGPT",
          "https://pl.wikipedia.org/wiki/PLLuM",
          "https://pl.wikipedia.org/wiki/Retrieval-augmented_generation",
          "https://pl.wikipedia.org/wiki/Hugging_Face",
          "https://pl.wikipedia.org/wiki/PyTorch"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/PLLuM",
          "similarity": 0.6841
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hugging_Face",
          "similarity": 0.5444
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Retrieval-augmented_generation",
          "similarity": 0.5342
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/PyTorch",
          "similarity": 0.5188
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/TensorFlow",
          "similarity": 0.5085
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.3105
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Blazar",
          "similarity": 0.3043
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.2993
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.2916
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Kwazar",
          "similarity": 0.283
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "similarity": 0.2787
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.2456
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.2437
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.2383
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "similarity": 0.2361
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.2135
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.2097
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.1989
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.1648
        }
      ],
      "nouns": {
        "chatgpt": 23,
        "model": 15,
        "rok": 8,
        "język": 8,
        "gpt": 7,
        "użytkownik": 6,
        "dane": 5,
        "listopad": 4,
        "odpowiedź": 4,
        "tekst": 4,
        "informacja": 4,
        "miesiąc": 4,
        "﻿": 4,
        "strona": 3,
        "chatbot": 3,
        "rozmowa": 3,
        "wiedza": 3,
        "uwaga": 3,
        "jakość": 3,
        "marzec": 3,
        "pytanie": 3,
        "zależność": 3,
        "treść": 3,
        "google": 3,
        "nazwa": 3,
        "program": 2,
        "openai": 2,
        "wersja": 2,
        "kwiecień": 2,
        "inteligencja": 2,
        "generować": 2,
        "podstawa": 2,
        "pomoc": 2,
        "uczyć": 2,
        "rozwój": 2,
        "luty": 2,
        "funkcja": 2,
        "narzędzie": 2,
        "pisać": 2,
        "robot": 2,
        "człowiek": 2,
        "generowanie": 2,
        "wyszukiwarka": 2,
        "naukowiec": 2,
        "stan": 2,
        "popularność": 2,
        "geminia": 2,
        "claude": 2,
        "anthropic": 2,
        "uniwersytet": 2,
        "badanie": 2,
        "alternatywa": 2,
        "maj": 2,
        "państwo": 2,
        "spółka": 2,
        "firma": 2,
        "trurl.ai": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Retrieval-augmented_generation",
      "chromadb_search": {
        "query_text": "Retrieval-augmented generation",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Retrieval-augmented_generation",
          "https://pl.wikipedia.org/wiki/ChatGPT",
          "https://pl.wikipedia.org/wiki/Hugging_Face",
          "https://pl.wikipedia.org/wiki/PLLuM",
          "https://pl.wikipedia.org/wiki/Dysk_akrecyjny"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/PLLuM",
          "similarity": 0.5037
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hugging_Face",
          "similarity": 0.3678
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Blazar",
          "similarity": 0.3579
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "similarity": 0.3307
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/TensorFlow",
          "similarity": 0.3284
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Kwazar",
          "similarity": 0.301
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "similarity": 0.2934
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/PyTorch",
          "similarity": 0.2882
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.2688
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.2331
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.2292
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.2085
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.1995
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.1843
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.1775
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.1597
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.1581
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.14
        }
      ],
      "nouns": {
        "dane": 5,
        "informacja": 4,
        "llm": 4,
        "użytkownik": 4,
        "pol": 4,
        "odpowiedź": 3,
        "retrieval": 2,
        "generation": 2,
        "zapytanie": 2,
        "model": 2,
        "źródło": 2,
        "dokument": 2,
        "technika": 2,
        "schemat": 2,
        "wzbogacać": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/PLLuM",
      "chromadb_search": {
        "query_text": "PLLuM",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/PLLuM",
          "https://pl.wikipedia.org/wiki/ChatGPT",
          "https://pl.wikipedia.org/wiki/PyTorch",
          "https://pl.wikipedia.org/wiki/Retrieval-augmented_generation",
          "https://pl.wikipedia.org/wiki/Pale_Blue_Dot"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/PyTorch",
          "similarity": 0.4379
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/TensorFlow",
          "similarity": 0.392
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hugging_Face",
          "similarity": 0.3832
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.2948
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.2898
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Blazar",
          "similarity": 0.2774
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "similarity": 0.2646
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.2571
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.253
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Kwazar",
          "similarity": 0.2454
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "similarity": 0.2192
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.2137
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.1835
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.1808
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.1759
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.1668
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.1558
        }
      ],
      "nouns": {
        "model": 8,
        "pllum": 7,
        "cyfryzacja": 4,
        "rok": 4,
        "język": 3,
        "projekt": 3,
        "ministerstwo": 2,
        "luty": 2,
        "rodzina": 2,
        "przetwarzać": 2,
        "tekst": 2,
        "ekspert": 2,
        "informatyka": 2,
        "instytut": 2,
        "pan": 2,
        "miliard": 2,
        "tworzyć": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Hugging_Face",
      "chromadb_search": {
        "query_text": "Hugging Face",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Hugging_Face",
          "https://pl.wikipedia.org/wiki/ChatGPT",
          "https://pl.wikipedia.org/wiki/Hekabe",
          "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "https://pl.wikipedia.org/wiki/PLLuM"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/PyTorch",
          "similarity": 0.4624
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/TensorFlow",
          "similarity": 0.4343
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.2126
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "similarity": 0.1858
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.1721
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Kwazar",
          "similarity": 0.1685
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.1671
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.1651
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.1353
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.1329
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.1324
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "similarity": 0.1275
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Blazar",
          "similarity": 0.1131
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.1098
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.0781
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.0743
        }
      ],
      "nouns": {
        "hugging": 2,
        "faca": 2,
        "firma": 2,
        "platforma": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/PyTorch",
      "chromadb_search": {
        "query_text": "PyTorch",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/PyTorch",
          "https://pl.wikipedia.org/wiki/TensorFlow",
          "https://pl.wikipedia.org/wiki/ChatGPT",
          "https://pl.wikipedia.org/wiki/Hugging_Face",
          "https://pl.wikipedia.org/wiki/PLLuM"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/TensorFlow",
          "similarity": 0.6975
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.2648
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Kwazar",
          "similarity": 0.2263
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.212
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Blazar",
          "similarity": 0.2106
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.1919
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.1869
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.1849
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "similarity": 0.1783
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "similarity": 0.1771
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.1701
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.1558
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.1092
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.0865
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.0734
        }
      ],
      "nouns": {
        "pytorch": 4,
        "język": 3,
        "biblioteka": 3,
        "python": 2,
        "linux": 2,
        "oprogramowanie": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/TensorFlow",
      "chromadb_search": {
        "query_text": "TensorFlow",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/TensorFlow",
          "https://pl.wikipedia.org/wiki/PyTorch",
          "https://pl.wikipedia.org/wiki/Hugging_Face",
          "https://pl.wikipedia.org/wiki/ChatGPT",
          "https://pl.wikipedia.org/wiki/PLLuM"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Blazar",
          "similarity": 0.2568
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.2508
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.2252
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Kwazar",
          "similarity": 0.2196
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "similarity": 0.2095
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "similarity": 0.1975
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.1742
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.1734
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.1519
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.1152
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.0901
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.0813
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.0803
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.0758
        }
      ],
      "nouns": {
        "biblioteka": 5,
        "język": 3,
        "c++": 3,
        "warstwa": 3,
        "model": 3,
        "google": 2,
        "listopad": 2,
        "wersja": 2,
        "python": 2,
        "api": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Kwazar",
      "chromadb_search": {
        "query_text": "Kwazar",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Kwazar",
          "https://pl.wikipedia.org/wiki/Blazar",
          "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "https://pl.wikipedia.org/wiki/Ciemna_energia"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Blazar",
          "similarity": 0.6909
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "similarity": 0.628
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.6149
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "similarity": 0.5948
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.4317
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.389
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.2405
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.2285
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.2256
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.2223
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.2105
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.2037
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.1725
        }
      ],
      "nouns": {
        "kwazar": 26,
        "rok": 12,
        "galaktyka": 11,
        "dziura": 9,
        "gwiazda": 8,
        "obiekt": 7,
        "kwazara": 7,
        "promieniowanie": 6,
        "ewolucja": 6,
        "czerwień": 4,
        "światło": 4,
        "miliard": 4,
        "dysk": 4,
        "odkrycie": 3,
        "niebo": 3,
        "jasność": 3,
        "czas": 3,
        "jądro": 3,
        "ziemia": 3,
        "odległościa": 3,
        "materia": 3,
        "otoczenie": 3,
        "stellar": 2,
        "źródło": 2,
        "moc": 2,
        "rodzaj": 2,
        "wiek": 2,
        "emisja": 2,
        "badać": 2,
        "rząd": 2,
        "centrum": 2,
        "dżet": 2,
        "zakres": 2,
        "przesunięć": 2,
        "liczba": 2,
        "tempo": 2,
        "powstawać": 2,
        "wykres": 2,
        "jednostka": 2,
        "objętość": 2,
        "związek": 2,
        "strona": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Blazar",
      "chromadb_search": {
        "query_text": "Blazar",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Blazar",
          "https://pl.wikipedia.org/wiki/Kwazar",
          "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "https://pl.wikipedia.org/wiki/Ciemna_energia"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "similarity": 0.6026
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "similarity": 0.5769
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.5634
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.3687
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.3244
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.2157
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.2149
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.1891
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.184
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.1657
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.1644
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.1495
        }
      ],
      "nouns": {
        "zakres": 13,
        "promieniowanie": 10,
        "blazara": 9,
        "blazar": 8,
        "}": 7,
        "dżet": 6,
        "lacertyd": 6,
        "typ": 5,
        "zmienność": 5,
        "kwazar": 4,
        "fala": 4,
        "polaryzacja": 4,
        "linia": 4,
        "położenie": 4,
        "∼": 4,
        "jądro": 4,
        "emisja": 4,
        "obserwacja": 4,
        "teleskop": 4,
        "czynnik": 4,
        "widmo": 3,
        "obserwator": 3,
        "kwazara": 3,
        "gamma": 3,
        "przypadek": 3,
        "energia": 3,
        "tev": 3,
        "składnik": 3,
        "jasność": 3,
        "charakter": 3,
        "mapa": 3,
        "obserwatorium": 3,
        "model": 3,
        "elektron": 3,
        "galaktyka": 2,
        "kąt": 2,
        "klasa": 2,
        "obiekt": 2,
        "bl": 2,
        "lac": 2,
        "rząd": 2,
        "dysk": 2,
        "szerokość": 2,
        "czerwień": 2,
        "odległościa": 2,
        "rok": 2,
        "skała": 2,
        "optycznie": 2,
        "długość": 2,
        "prędkoścei": 2,
        "kampania": 2,
        "dane": 2,
        "interpretacja": 2,
        "widm": 2,
        "plazma": 2,
        "propagacja": 2,
        "=": 2,
        "galaktyk": 2,
        "radiogalaktyk": 2,
        "fr": 2,
        "obecność": 2,
        "pole": 2,
        "proces": 2,
        "komptonizacja": 2,
        "foton": 2,
        "obszar": 2,
        "\\displaystyle": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
      "chromadb_search": {
        "query_text": "Dysk akrecyjny",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "https://pl.wikipedia.org/wiki/Zbrojni",
          "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "https://pl.wikipedia.org/wiki/Agamemnon"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.5898
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "similarity": 0.5601
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.462
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.3921
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.3468
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.3121
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.2822
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.2612
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.258
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.2338
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.2115
        }
      ],
      "nouns": {
        "gwiazda": 26,
        "dysk": 25,
        "materia": 15,
        "dziura": 15,
        "promieniowanie": 12,
        "część": 10,
        "moment": 9,
        "pole": 8,
        "obiekt": 6,
        "pąd": 6,
        "horyzont": 6,
        "układ": 6,
        "lepkość": 5,
        "galaktyka": 5,
        "powierzchnia": 5,
        "efekt": 5,
        "}": 5,
        "akrecja": 4,
        "pęd": 4,
        "orbity": 4,
        "temperatura": 4,
        "odległość": 4,
        "promień": 4,
        "cząstka": 4,
        "foton": 4,
        "typ": 3,
        "wynik": 3,
        "energia": 3,
        "zakres": 3,
        "dyski": 3,
        "miejsce": 3,
        "ciało": 3,
        "wiatr": 3,
        "plazma": 3,
        "teoria": 3,
        "względność": 3,
        "kierunek": 3,
        "struktura": 2,
        "pył": 2,
        "gaz": 2,
        "zjawisko": 2,
        "siła": 2,
        "orbita": 2,
        "obszar": 2,
        "ciepło": 2,
        "cząstek": 2,
        "kwazara": 2,
        "’": 2,
        "przypadek": 2,
        "ruch": 2,
        "ośrodek": 2,
        "przekazywać": 2,
        "wpływ": 2,
        "forma": 2,
        "\\frac": 2,
        "promieć": 2,
        "masa": 2,
        "podstawa": 2,
        "wartość": 2,
        "czas": 2,
        "obserwator": 2,
        "rozkład": 2,
        "świecenie": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
      "chromadb_search": {
        "query_text": "Soczewkowanie grawitacyjne",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "https://pl.wikipedia.org/wiki/Retrieval-augmented_generation",
          "https://pl.wikipedia.org/wiki/Blazar",
          "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "https://pl.wikipedia.org/wiki/ChatGPT"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "similarity": 0.6473
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.4663
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.4043
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.2773
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.2282
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.2116
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.2056
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.1908
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.1716
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.1666
        }
      ],
      "nouns": {
        "obraz": 18,
        "galaktyka": 15,
        "soczewkowanie": 12,
        "źródło": 8,
        "obiekt": 8,
        "soczewka": 8,
        "efekt": 7,
        "materia": 6,
        "ciało": 4,
        "soczewkowania": 4,
        "hubbel": 4,
        "’": 4,
        "promień": 3,
        "gromada": 3,
        "przykład": 3,
        "mikrosoczewkować": 3,
        "symulacja": 3,
        "łuk": 3,
        "teleskop": 3,
        "szczególność": 3,
        "masa": 2,
        "pojaśnieć": 2,
        "przejaw": 2,
        "soczewkowaniu": 2,
        "parametr": 2,
        "gęstość": 2,
        "wszechświat": 2,
        "rodzaj": 2,
        "obserwator": 2,
        "einstein": 2,
        "rozkład": 2,
        "zderzenie": 2,
        "energia": 2,
        "podstawa": 2,
        "zniekształceć": 2,
        "analiza": 2,
        "obecność": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Ciemna_energia",
      "chromadb_search": {
        "query_text": "Ciemna energia",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "https://pl.wikipedia.org/wiki/Blazar",
          "https://pl.wikipedia.org/wiki/Kwazar",
          "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.4949
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.4021
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Zbrojni",
          "similarity": 0.2921
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.2866
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.2669
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.2443
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.2123
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.2013
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.1954
        }
      ],
      "nouns": {
        "energia": 15,
        "wszechświat": 13,
        "ekspansja": 9,
        "materia": 7,
        "galaktyka": 7,
        "pomiar": 6,
        "promieniowanie": 6,
        "model": 6,
        "gęstość": 6,
        "tempo": 5,
        "masa": 5,
        "teoria": 5,
        "obserwacja": 5,
        "rozkład": 5,
        "kosmos": 4,
        "problem": 4,
        "wynik": 4,
        "część": 4,
        "tło": 4,
        "wszechświet": 3,
        "istnieć": 3,
        "założenie": 3,
        "wiek": 3,
        "rok": 3,
        "typ": 3,
        "ziemia": 3,
        "hipoteza": 3,
        "dane": 3,
        "struktura": 3,
        "gwiazda": 2,
        "dark": 2,
        "energy": 2,
        "rozszerzać": 2,
        "badanie": 2,
        "siła": 2,
        "pole": 2,
        "kwintesencja": 2,
        "form": 2,
        "obraz": 2,
        "czas": 2,
        "kosmologia": 2,
        "cdm": 2,
        "aspekt": 2,
        "względność": 2,
        "układ": 2,
        "przegląd": 2,
        "soczewkowanie": 2,
        "oscylacja": 2,
        "parametr": 2,
        "czerwień": 2,
        "gromada": 2,
        "hubbel": 2,
        "’": 2,
        "miejsce": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Zbrojni",
      "chromadb_search": {
        "query_text": "Zbrojni",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Zbrojni",
          "https://pl.wikipedia.org/wiki/PLLuM",
          "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "https://pl.wikipedia.org/wiki/Blazar"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "similarity": 0.6829
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.3575
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.3442
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.335
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.3162
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.3134
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.2407
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.2087
        }
      ],
      "nouns": {
        "’": 4,
        "straż": 4,
        "eath": 3,
        "powieść": 2,
        "wydanie": 2,
        "data": 2,
        "wydać": 2,
        "Piotr": 2,
        "cholew": 2,
        "potomek": 2,
        "ankh": 2,
        "czas": 2,
        "śledztwo": 2,
        "nazwa": 2,
        "oryginał": 2,
        "funkcja": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Muzyka_duszy",
      "chromadb_search": {
        "query_text": "Muzyka duszy",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Muzyka_duszy",
          "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "https://pl.wikipedia.org/wiki/Zbrojni",
          "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "https://pl.wikipedia.org/wiki/Ciemna_energia"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.2956
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.2762
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "similarity": 0.2758
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.2543
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.2466
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.2416
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.1802
        }
      ],
      "nouns": {
        "muzyka": 5,
        "dusza": 4,
        "powieść": 3,
        "wydanie": 3,
        "data": 2,
        "wydać": 2,
        "Piotr": 2,
        "cholew": 2,
        "dysk": 2,
        "książka": 2,
        "śmierć": 2,
        "imp": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
      "chromadb_search": {
        "query_text": "Pale Blue Dot",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "https://pl.wikipedia.org/wiki/Dysk_akrecyjny",
          "https://pl.wikipedia.org/wiki/Ciemna_energia"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "similarity": 0.7369
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.3068
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.2773
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.2335
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.2238
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.1881
        }
      ],
      "nouns": {
        "zdjęcie": 7,
        "ziemia": 6,
        "układ": 6,
        "sonda": 5,
        "voyager": 4,
        "odległość": 3,
        "miliard": 3,
        "dot": 2,
        "kilometr": 2,
        "wersja": 2,
        "użyć": 2,
        "fotografia": 2,
        "rok": 2,
        "cel": 2,
        "aparat": 2,
        "obszar": 2,
        "portret": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/The_Blue_Marble",
      "chromadb_search": {
        "query_text": "The Blue Marble",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/The_Blue_Marble",
          "https://pl.wikipedia.org/wiki/Pale_Blue_Dot",
          "https://pl.wikipedia.org/wiki/Soczewkowanie_grawitacyjne",
          "https://pl.wikipedia.org/wiki/Ciemna_energia",
          "https://pl.wikipedia.org/wiki/Kwazar"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.279
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.2615
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.224
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Hekabe",
          "similarity": 0.2043
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.194
        }
      ],
      "nouns": {
        "zdjęcie": 13,
        "fotografia": 6,
        "obraz": 5,
        "ziemia": 5,
        "apollo": 4,
        "marble": 4,
        "nasa": 4,
        "załoga": 3,
        "afryka": 3,
        "góra": 3,
        "grudzień": 3,
        "rozdzielczość": 3,
        "rok": 3,
        "blue": 3,
        "raz": 2,
        "chmura": 2,
        "nazwa": 2,
        "seria": 2,
        "zestaw": 2,
        "astronauta": 2,
        "historia": 2,
        "pełnia": 2,
        "hasselblad": 2,
        "harrison": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Hekabe",
      "chromadb_search": {
        "query_text": "Hekabe",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Hekabe",
          "https://pl.wikipedia.org/wiki/Penelopa",
          "https://pl.wikipedia.org/wiki/Achilles",
          "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "https://pl.wikipedia.org/wiki/Hugging_Face"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.6559
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Achilles",
          "similarity": 0.64
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.5535
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.5102
        }
      ],
      "nouns": {
        "dziecko": 3,
        "królowa": 2,
        "żona": 2,
        "priam": 2,
        "polidor": 2,
        "kasandra": 2,
        "kreuza": 2,
        "śmierć": 2,
        "rozpacz": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Achilles",
      "chromadb_search": {
        "query_text": "Achilles",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Achilles",
          "https://pl.wikipedia.org/wiki/Agamemnon",
          "https://pl.wikipedia.org/wiki/Hekabe",
          "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "https://pl.wikipedia.org/wiki/Penelopa"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Agamemnon",
          "similarity": 0.6951
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.6037
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.5663
        }
      ],
      "nouns": {
        "achilles": 8,
        "heros": 3,
        "syn": 3,
        "matka": 3,
        "hektora": 3,
        "ἀχιλλεύς": 2,
        "mitologia": 2,
        "peleus": 2,
        "ojciec": 2,
        "dziecko": 2,
        "bohater": 2,
        "wojna": 2,
        "myrmidon": 2,
        "król": 2,
        "miasto": 2,
        "tetyda": 2,
        "cios": 2,
        "miejsce": 2,
        "Achilles": 2,
        "córka": 2,
        "branek": 2,
        "potrzebny": 2,
        "przypis": 2,
        "śmierć": 2,
        "patroklos": 2,
        "zbroję": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Agamemnon",
      "chromadb_search": {
        "query_text": "Agamemnon",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Agamemnon",
          "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "https://pl.wikipedia.org/wiki/Penelopa",
          "https://pl.wikipedia.org/wiki/Hekabe",
          "https://pl.wikipedia.org/wiki/Achilles"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "similarity": 0.618
        },
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.5513
        }
      ],
      "nouns": {
        "agamemnon": 15,
        "żona": 7,
        "czas": 7,
        "klitajmestra": 7,
        "udział": 5,
        "król": 3,
        "menelaos": 3,
        "syn": 3,
        "agamemnona": 3,
        "córka": 3,
        "wojna": 3,
        "kasandra": 3,
        "głowa": 2,
        "argolid": 2,
        "plejstenes": 2,
        "dziecko": 2,
        "chrysotemis": 2,
        "elektra": 2,
        "ifianassa": 2,
        "orestes": 2,
        "helena": 2,
        "ifigenia": 2,
        "wyprawa": 2,
        "oblężenie": 2,
        "branka": 2,
        "chryzejda": 2,
        "iliada": 2,
        "achilles": 2,
        "ręka": 2,
        "kasandro": 2,
        "śmierć": 2,
        "ajgistos": 2,
        "mąż": 2,
        "dom": 2,
        "kąpiel": 2,
        "sieć": 2,
        "topór": 2,
        "wersja": 2,
        "agamemnić": 2,
        "dzień": 2
      }
    },
    {
      "url1": "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
      "chromadb_search": {
        "query_text": "Sen Agamemnona",
        "top_5_results": [
          "https://pl.wikipedia.org/wiki/Agamemnon",
          "https://pl.wikipedia.org/wiki/Sen_Agamemnona",
          "https://pl.wikipedia.org/wiki/Hekabe",
          "https://pl.wikipedia.org/wiki/Achilles",
          "https://pl.wikipedia.org/wiki/Penelopa"
        ]
      },
      "similarities": [
        {
          "url2": "https://pl.wikipedia.org/wiki/Penelopa",
          "similarity": 0.5139
        }
      ],
      "nouns": {
        "sen": 14,
        "zeus": 4,
        "schemat": 3,
        "poemat": 2,
        "ptaszę": 2,
        "wąż": 2,
        "achaj": 2,
        "homer": 2,
        "przekazać": 2,
        "wiadomość": 2,
        "cambier": 2,
        "apokalipsa": 2
      }
    }
  ]
}
```
